...
router.post("/add", upload.fields([
    ...
    if (req.files.changeMainImage) {
        ...
        const imagePath = `${DOMAIN}/img/products/` + req.files.changeMainImage[0].filename;
    ...
    if (req.files.relatedImages) {
        const DOMAIN = req.protocol + '://' + req.get('host');
        const relatedImages = req.files.relatedImages.map(file => `${DOMAIN}/img/products/` + file.filename);
